...
const DATA_CLI_NOTE = [
  ...
  "纪律：",
  "1) 它只能查到当前账号自己的数据（**别人的分身**查不到，只有查询、没有删除和修改）。",
  // ⚠️ 例外且必须讲清楚：会话列表里会混进「别人分享给他的分身」的会话。
  // 不讲的话模型看到「这个名字在 avatars 里查不到」就会自己推断成「分身已被删除」——
  // 2026-09-20 用户实测被这么误导过（那条其实是他聊过的、别人分享的分身）。
  "2) `convs` 里带「（他人分享的分身，未被删除）」标记的，是**别人分享给他、他聊过的分身**，"
    + "**不是被删除的分身**。绝不能把它说成「分身已被删除」「他删过这个分身」——"
    + "它不在 `avatars` 里只是因为 `avatars` 只列他自己的分身，这很正常。",
  "3) 想知道某个知识库文档讲了什么，用 `ask` 让分身自己概括。**不要去读文件原文**：那些文档不在本机磁盘上，而且大文件会把上下文撑爆。",
  "4) 命令返回本身有长度上限，看到「已截断」就把问题问得更具体，不要反复拉全量。",
  "5) 用户没提到他自己的数据时，不用主动调用。",
  // 分身能不能继续投喂，看 avatars 的 canFeed / status / remainingMb —— 直接把判据交代清楚，
  // 否则模型看到 online:false 只会说「离线了」，而「已锁定」用户照着做也恢复不了。
  "6) 分身能不能再喂资料看 `avatars` 里的 `canFeed`：`status=locked` 表示它因会员权益过期被系统下线（要让用户先在 App 里恢复它），`remainingMb=0` 表示知识库已满（要先清理旧资料）。**投喂只能由用户在「工作台 → 文件」页里点，本机命令不能喂。**",
  // 输出里的 [数字] 只是**给你接着下一条命令用的句柄**。模型很容易把它当成「信息」原样念给用户，
  // 用户看到 3 / 88 / 991 这种编号只会困惑。技术字段（MB、来源枚举之类）同理。
